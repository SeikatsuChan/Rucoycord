...
client.on("ready", () => {
  const mainGuild = client.guilds.get(config.SERVER.toString());
  console.log(
    `Logged in! ${client.user.username} is active in ${client.guilds.size} server(s)`
  );
  client.user.setActivity(`${prefix}help`, { type: "WATCHING" });

  setInterval(() => {
    if (config.CLASS_ROLES === "true") {
      client.players.forEach((values, user) => {
        if (mainGuild.members.has(user)) {
          //if(client.players.get(user, "supporter") == true) {
          if (
            client.players.get(user, "supporter") == true &&
            mainGuild.members.get(user).roles.has(config.STANDARD_KNIGHT)
          ) {
            console.log(client.players.get(user, "user"));
            mainGuild.members.get(user).addRole(config.SUPPORTER_KNIGHT);
            mainGuild.members.get(user).removeRole(config.STANDARD_KNIGHT);
          } else if (
            client.players.get(user, "supporter") == true &&
            mainGuild.members.get(user).roles.has(config.STANDARD_ARCHER)
          ) {
            mainGuild.members.get(user).addRole(config.SUPPORTER_ARCHER);
            mainGuild.members.get(user).removeRole(config.STANDARD_ARCHER);
          } else if (
            client.players.get(user, "supporter") == true &&
            mainGuild.members.get(user).roles.has(config.STANDARD_MAGE)
          ) {
            mainGuild.members.get(user).addRole(config.SUPPORTER_MAGE);
            mainGuild.members.get(user).removeRole(config.STANDARD_MAGE);
          }
          // }
        }
      });
    ...
    if (config.AUTOROLE === "true") {
      client.players.forEach((values, user) => {
        if (mainGuild.members.has(user)) {
          ...
          if (subStatus == true && config.ROLE_SUB === "true") {
            var today = new Date();
            ...
            if (client.players.get(user, "sub_date") === today) {
              if (
                client.players.get(user, "diamonds") <
                Number(config.ROLE_COST)
              ) {
                mainGuild.roles
                  .get(client.players.get(user, "sub_role"))
                  .delete();
                ...
                client.players.set(
                  user,
                  client.user.get(user, "diamonds") - 1000,
                  "diamonds"
                );
          ...
          if (charname !== "not-linked") {
            request(
              `https://www.rucoyonline.com/characters/${charname}`,
              (error, response, html) => {
                if (!error && response.statusCode == 200) {
                  ...
                  if (
                    charHeading.includes("Supporter") &&
                    client.players.get(user, "supporter") == false
                  ) {
                    client.players.set(user, true, "supporter");
                    mainGuild.members
                      .get(client.players.get(user, "user"))
                      .addRole(config.SUPPORTERROLE);
                    if (config.SUPPORTER_NOTIFS === "true") {
                      mainGuild.channels
                        .get(config.SUPPORTER_NOTIF_CHANNEL)
                        .send(`<@${user}> gained supporter status.`);
                    }
                    if (
                      config.ECONOMY === "true" &&
                      config.SUPPORTER_DIAMONDS === "true"
                    ) {
                      client.players.set(
                        user,
                        client.players.get(user, "diamonds") +
                          parseInt(config.SUPPORTER_DIAMONDS_NUMBER),
                        "diamonds"
                      );
                    }
                  }
                  ...
                  if (
                    lvl >= 600 &&
                    client.players.get(user, "levelrole") != 600
                  ) {
                    mainGuild.members
                      .get(client.players.get(user, "user"))
                      .addRole(config.LEVEL600ROLE);
                    client.players.set(user, 600, "levelrole");
                  } else if (
                    lvl == 500 &&
                    client.players.get(user, "levelrole") != 500
                  ) {
                    mainGuild.members
                      .get(client.players.get(user, "user"))
                      .addRole(config.LEVEL500ROLE);
                    client.players.set(user, 500, "levelrole");
                  } else if (
                    lvl == 400 &&
                    client.players.get(user, "levelrole") != 400
                  ) {
                    mainGuild.members
                      .get(client.players.get(user, "user"))
                      .addRole(config.LEVEL400ROLE);
                    client.players.set(user, 400, "levelrole");
                  } else if (
                    lvl == 300 &&
                    client.players.get(user, "levelrole") != 300
                  ) {
                    mainGuild.members
                      .get(client.players.get(user, "user"))
                      .addRole(config.LEVEL300ROLE);
                    client.players.set(user, 300, "levelrole");
                  } else if (
                    lvl == 200 &&
                    client.players.get(user, "levelrole") != 200
                  ) {
                    mainGuild.members
                      .get(client.players.get(user, "user"))
                      .addRole(config.LEVEL200ROLE);
                    client.players.set(user, 200, "levelrole");
                  } else if (
                    lvl == 100 &&
                    client.players.get(user, "levelrole") != 100
                  ) {
                    mainGuild.members
                      .get(client.players.get(user, "user"))
                      .addRole(config.LEVEL100ROLE);
...
client.on("voiceStateUpdate", (oldMember, newMember) => {
  if (config.VCROLE === "true") {
    let newUserChannel = newMember.voiceChannel;
    let oldUserChannel = oldMember.voiceChannel;
    if (oldUserChannel === undefined && newUserChannel !== undefined) {
      newMember.addRole(config.VCROLEID);
    } else if (newUserChannel === undefined) {
      newMember.removeRole(config.VCROLEID);
    }
  }
});
...
client.on("message", async message => {
  ...
  if (config.ECONOMY === "true")
    client.players.math(key, "+", Math.floor(Math.random() * 2), "diamonds");
  if (message.content.startsWith(prefix)) {
    ...
    if (command === "link") {
      if (
        !message.guild.members
          ...
          .roles.has(config.ADMIN_ROLE) &&
        config.FREE_LINKING === "false"
      )
      ...
      {
        let member = "none";
        if (config.FREE_LINKING === "true") {
          member = message.author.id;
        } else {
          member = message.mentions.users.first().id;
          args.shift();
        }
        if (member === "none" || !args[0])
        ...
        try {
          ...
          message.channel.send(
            `Successfully linked **${args.join(" ")}** to <@${member}>`
          );
        } catch (error) {
          message.channel.send(
            `There was an issue linking:\n\`\`\`js\n${error}\`\`\``
          );
        }
      }
    }
    if (command === "player") {
      if (!args) {
        let embed = new Discord.RichEmbed()
          ...
          .setDescription(
            "Proper usage is `" +
              prefix +
              'player Name`, where "Name" is the name of the player you\'re looking for.'
          );
      ...
      if (message.mentions.users.first()) {
        if (
          !client.players.has(message.mentions.users.first().id) ||
          client.players.get(message.mentions.users.first().id, "ign") ===
            "not-linked"
        )
          return message.channel.send(
            `**${
      ...
      request(
        `https://www.rucoyonline.com/characters/${char}`,
        (error, response, html) => {
          if (!error && response.statusCode == 200) {
            const $ = cheerio.load(html);
            ...
            if (charHeading.text().includes("Game Master")) {
              ...
              var charEmbed = new Discord.RichEmbed()
                ...
                .addField(
                  ...
                  `[${
                    character.guild
                  }](https://www.rucoyonline.com/guild/${character.guild.replaceAll(
                    " ",
                    "%20"
                  )})`
                )
    ...
    if (command === "pvptest" && message.author.id == config.OWNER) {
      let char = args.join(" ");
      if (message.mentions.users.first()) {
        if (
          !client.players.has(message.mentions.users.first().id) ||
        ...
        (error, response, html) => {
          if (!error && response.statusCode == 200) {
            ...
            if (newStr.indexOf("killed") > -1) {
              if (
                newStr[newStr.indexOf("killed") + 1].includes(character.name)
              ) {
    ...
    if (command === "guild") {
      if (!args) {
        let embed = new Discord.RichEmbed()
          ...
          .setDescription(
            "Proper usage is `" +
              prefix +
              'guild Name`, where "Name" is the name of the guild you\'re looking for.'
          );
      ...
      request(
        `https://www.rucoyonline.com/guild/${urlname}`,
        (error, response, html) => {
          if (!error && response.statusCode == 200) {
            const $ = cheerio.load(html);
            const guildName = $("h3").text();
            const guildDescription = $("p")
              .text()
              .substring(
                0,
                $("p")
            ...
            const guildFounded = $("p")
              .text()
              .substring(
                $("p")
                  .text()
                  .indexOf("Founded on") + 11,
                $("p").text().length
              );
            ...
            let guildEmbed = new Discord.RichEmbed()
              ...
              .addField(
                "Leader",
                `[${leader}](https://www.rucoyonline.com/characters/${leader.replaceAll(
                  " ",
                  "%20"
                )})`,
                true
            ...
            return message.channel.send(
              "There was an error finding that guild:\n```" + error + "```"
            );
        }
      );
    }
      ...
      request("https://www.rucoyonline.com", (error, response, html) => {
        if (!error && response.statusCode == 200) {
          ...
          let countEmbed = new Discord.RichEmbed()
            ...
            .setThumbnail(
              "https://www.rucoyonline.com/assets/favicon/favicon-32x32-b4cafe4c726eace2f4165a0f0d185266103ba79598a894886a312e9e6effaa9a.png"
            );

          message.channel.send(countEmbed);
        } else {
          return message.channel.send(
            "There was an error connecting to rucoyonline.com:\n```" +
              error +
              "```"
          );
        }
    ...
    if (
      ...
      let ptsEmbed = new Discord.RichEmbed()
        ...
        .setDescription(
          `${config.CURRENCY}**${commaSeparateNumber(
            client.players.get(player.id, "diamonds")
          )}**`
        )
        .setColor("#ff0000");
    ...
    if (command === "give" && config.ECONOMY === "true") {
      if (!message.mentions.users.first() || !args[1])
        return message.channel.send(
          "Missing args, proper usage is `" + prefix + "give @user 100`"
        );
      ...
      let notake = false;
      if (
        message.content.includes("-admin") &&
        message.guild.members
          .get(message.author.id)
          .roles.has(config.ADMIN_ROLE)
      )
        notake = true;
      if (!/^\d+$/.test(giveDiamonds) && !notake)
        return message.channel.send("You need to specify a **number**!");
      if (giveDiamonds < 1 && !notake)
        return message.channel.send("You can't give an amount less than 1");
      if (
        giveDiamonds > client.players.get(message.author.id, "diamonds") &&
        !notake
      )
        return message.channel.send("You don't have enough diamonds to give!");

      client.players.set(
        userTwo,
        client.players.get(userTwo, "diamonds") + parseInt(giveDiamonds),
        "diamonds"
      );
      if (!notake)
        client.players.set(
          userOne,
          client.players.get(userOne, "diamonds") - parseInt(giveDiamonds),
          "diamonds"
        );

      let giveEmbed = new Discord.RichEmbed()
        .setDescription(
          `<@${message.author.id}> gifted ${
            config.CURRENCY
          }**${commaSeparateNumber(parseInt(giveDiamonds))}** to <@${userTwo}>!`
        )
        .setColor("#26CB7C");
    ...
    if (
      (command === "lb" || command === "leaderboard") &&
      config.ECONOMY === "true"
    ) {
      ...
      for (const data of top10) {
        embed.addField(
          ...
          `${config.CURRENCY}${commaSeparateNumber(data.diamonds)}`
        );
      }
      return message.channel.send({ embed });
    }
    if (command === "subscribe" && config.ROLE_SUB === "true") {
      if (
        client.players.get(message.author.id, "diamonds") <
        Number(config.ROLE_COST)
      )
        return message.channel.send(
          "You don't have enough diamonds to subscribe! You need " +
            commaSeparateNumber(Number(config.ROLE_COST))
        );
      if (client.players.get(message.author.id, "sub") == true)
        return message.channel.send("You are already subscribed!");
      ...
      message.channel.send(
        "All done! You will be charged " +
          commaSeparateNumber(Number(config.ROLE_COST)) +
          " diamonds every month. To edit your role, type /subrole [name/color] (name/hex). To end your subscription, type /unsubscribe (NOTE: Your diamonds will not be refunded if you do this.)"
      );
    }
    if (command === "unsubscribe" && config.ROLE_SUB === "true") {
      if (client.players.get(message.author.id, "sub") == false)
        return message.channel.send("You aren't subscribed!");

      message.guild.roles
        .get(client.players.get(message.author.id, "sub_role"))
        .delete();
    ...
    if (command === "subrole" && config.ROLE_SUB === "true") {
      if (client.players.get(message.author.id, "sub") == false)
        return message.channel.send(
          "You need to subscribe to use this command!"
        );
      ...
      if (args[0] === "color") {
        ...
        return message.channel.send("There was an issue... ping Seikatsu.");
    }
	if(command === "reboot" && message.author.id === config.OWNER) {
